...
resource "aws_iam_role" "lambda" {
  name  = "${var.serverless_app_project_name}-lambda-role"
...
resource "aws_iam_policy" "lambda" {
  name        = "${var.serverless_app_project_name}-lambda-policy"
  description = "Managed policy for serverless-app Lambda functions"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "${aws_s3_bucket.serverless_app_images.arn}/*"
      }
    ]
  })
...
resource "aws_cloudwatch_log_group" "create_post" {
  name              = "/aws/lambda/${var.serverless_app_project_name}-CreatePostFunction"
  retention_in_days = 30
  tags              = local.serverless_app_tags
}

resource "aws_cloudwatch_log_group" "list_posts" {
  name              = "/aws/lambda/${var.serverless_app_project_name}-ListPostsFunction"
  retention_in_days = 30
  tags              = local.serverless_app_tags
}

resource "aws_cloudwatch_log_group" "delete_post" {
  name              = "/aws/lambda/${var.serverless_app_project_name}-DeletePostFunction"
  retention_in_days = 30
  tags              = local.serverless_app_tags
}

resource "aws_cloudwatch_log_group" "get_upload_urls" {
  name              = "/aws/lambda/${var.serverless_app_project_name}-GetUploadUrlsFunction"
  retention_in_days = 30
  tags              = local.serverless_app_tags
}

resource "aws_cloudwatch_log_group" "profile" {
  name              = "/aws/lambda/${var.serverless_app_project_name}-ProfileFunction"
  retention_in_days = 30
  tags              = local.serverless_app_tags
}
...
data "archive_file" "lambda_create" {
  type        = "zip"
  source_dir  = local.serverless_app_dist_dir
  output_path = "${local.terraform_cache_dir}/lambda/create.zip"
...
data "archive_file" "lambda_list" {
  type        = "zip"
  source_dir  = local.serverless_app_dist_dir
  output_path = "${local.terraform_cache_dir}/lambda/list.zip"
  excludes    = ["node_modules"]
}
...
data "archive_file" "lambda_delete" {
  type        = "zip"
  source_dir  = local.serverless_app_dist_dir
  output_path = "${local.terraform_cache_dir}/lambda/delete.zip"
  excludes    = ["node_modules"]
}
...
data "archive_file" "lambda_get_upload_urls" {
  type        = "zip"
  source_dir  = local.serverless_app_dist_dir
  output_path = "${local.terraform_cache_dir}/lambda/getuploadurls.zip"
  excludes    = ["node_modules"]
}
...
data "archive_file" "lambda_profile" {
  type        = "zip"
  source_dir  = local.serverless_app_dist_dir
  output_path = "${local.terraform_cache_dir}/lambda/profile.zip"
  excludes    = ["node_modules"]
}

resource "aws_lambda_function" "create_post" {
  filename         = data.archive_file.lambda_create.output_path
  function_name    = "${var.serverless_app_project_name}-CreatePostFunction"
  ...
  environment {
    variables = {
      ...
      IMAGES_BUCKET_NAME = aws_s3_bucket.serverless_app_images.bucket
      LOG_LEVEL          = "info"
      NODE_ENV           = "production"
    }
  }
...
resource "aws_lambda_function" "list_posts" {
  ...
  function_name    = "${var.serverless_app_project_name}-ListPostsFunction"
  role             = aws_iam_role.lambda.arn
...
resource "aws_lambda_function" "delete_post" {
  ...
  function_name    = "${var.serverless_app_project_name}-DeletePostFunction"
  role             = aws_iam_role.lambda.arn
...
resource "aws_lambda_function" "get_upload_urls" {
  ...
  function_name    = "${var.serverless_app_project_name}-GetUploadUrlsFunction"
  role             = aws_iam_role.lambda.arn
...
resource "aws_lambda_function" "profile" {
  ...
  function_name    = "${var.serverless_app_project_name}-ProfileFunction"
  role             = aws_iam_role.lambda.arn
