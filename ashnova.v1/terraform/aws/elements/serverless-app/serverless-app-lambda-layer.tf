...
resource "null_resource" "lambda_layer_build" {
  triggers = {
    ...
    package_json      = filemd5("${local.serverless_app_dir}/package.json")
    package_lock_json = fileexists("${local.serverless_app_dir}/package-lock.json") ? filemd5("${local.serverless_app_dir}/package-lock.json") : ""
  }
...
rm -rf ${local.terraform_cache_dir}/layer
mkdir -p ${local.terraform_cache_dir}/layer/nodejs

# Copy package files
cp ${local.serverless_app_dir}/package.json ${local.terraform_cache_dir}/layer/nodejs/
if [ -f ${local.serverless_app_dir}/package-lock.json ]; then
  cp ${local.serverless_app_dir}/package-lock.json ${local.terraform_cache_dir}/layer/nodejs/
fi

# Install production dependencies only
cd ${local.terraform_cache_dir}/layer/nodejs
...
data "archive_file" "lambda_layer" {
  type        = "zip"
  source_dir  = "${local.terraform_cache_dir}/layer"
  output_path = "${local.terraform_cache_dir}/lambda-layer.zip"
...
resource "aws_lambda_layer_version" "nodejs_dependencies" {
  layer_name          = "${var.serverless_app_project_name}-nodejs-dependencies"
  description         = "Node.js dependencies for ${var.serverless_app_project_name} Lambda functions"
