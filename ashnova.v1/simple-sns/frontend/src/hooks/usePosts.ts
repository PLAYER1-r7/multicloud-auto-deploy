...
const fetchPosts = async (limit: number = 20, nextToken?: string, tag?: string): Promise<{ items: Post[]; nextToken?: string }> => {
  ...
  const { res, json } = await apiFetch(`/posts?${params.toString()}`, {
    method: 'GET',
  });
  ...
  return { items: validated.items, nextToken: validated.nextToken };
};

/**
 * Get upload URLs for S3 presigned upload
 */
const getUploadUrls = async (count: number): Promise<{ postId: string; urls: Array<{ url: string; key: string }>; expiresIn: number }> => {
  const { res, json } = await apiFetch('/upload-urls', {
    method: 'POST',
 ...
 * Create a new post
 */
const createPost = async (data: { content: string; imageKeys?: string[]; isMarkdown?: boolean; tags?: string[] }): Promise<void> => {
