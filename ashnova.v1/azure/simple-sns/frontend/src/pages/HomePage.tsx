...
export const HomePage: React.FC = () => {
  const { isAuthenticated, login, logout, getAccessToken, getUserEmail } = useMsalAuth();
  ...
  // Update token in localStorage when authenticated
  useEffect(() => {
    if (isAuthenticated) {
      ...
      getAccessToken()
        .then(token => {
          console.log('HomePage: Token received:', token ? 'YES' : 'NO');
          console.log('HomePage: Token length:', token?.length || 0);
          tokenValue = token;
          if (token) {
            console.log('HomePage: About to set token...');
            try {
              setToken(token);
              console.log('HomePage: Token set successfully');
            } catch (e) {
              console.error('HomePage: setToken threw error:', e);
            }
          } else {
            console.log('HomePage: No token to set');
          }
          return token;
        })
        .catch(err => {
          // MSALのバリデーションエラーは無視し、取得したトークンは設定する
          console.log('HomePage: Caught error:', err?.message || err);
          console.log('HomePage: tokenValue in catch:', tokenValue ? 'EXISTS' : 'NULL');
          console.log('HomePage: tokenValue length in catch:', tokenValue?.length || 0);
          if (tokenValue) {
            console.log('HomePage: Setting token despite error...');
            try {
              setToken(tokenValue);
              console.log('HomePage: Token set despite error');
            } catch (e) {
              console.error('HomePage: setToken in catch threw error:', e);
            }
          } else {
            console.log('HomePage: No tokenValue to set in catch');
          }
          // Promise を正常に解決して、unhandled rejection を防ぐ
          return undefined;
        });
    }
