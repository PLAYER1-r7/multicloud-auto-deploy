...
## ルール18 — Azure Front Door Premium SKU の利用を禁止する

インフラコスト抑制のため、このリポジトリでは Azure Front Door の Premium SKU を**使用禁止**とする。
...
- Azure CLI / IaC で Front Door を Premium として新規作成・更新する変更

Premium 相当の機能が必要に見える場合でも、まず以下を優先すること:

1. Standard SKU で実現可能な構成に落とし込む（例: WAF Policy の明示的な関連付け）。
2. 月額コストを抑えられる代替マネージドサービスへ設計変更する。
3. コスト例外の Issue を作成し、オーナーの明示承認を得るまで変更しない。
