...
def _get_container():
    ...
    _cosmos_client = CosmosClient(
        config.COSMOS_DB_ENDPOINT, credential=config.COSMOS_DB_KEY)
    database = _cosmos_client.get_database_client(config.COSMOS_DB_DATABASE)
    _container_client = database.get_container_client(
        config.COSMOS_DB_CONTAINER)
    return _container_client
...
def _blob_url(key: str, permission: BlobSasPermissions, expiry_seconds: int) -> str:
    ...
    return (
        ...
        f"{config.AZURE_STORAGE_CONTAINER}/{key}?{sas}"
    )
...
def _build_image_urls(keys: list[str]) -> list[str] | None:
    ...
    try:
        return [
            _blob_url(key, BlobSasPermissions(
                read=True), config.SIGNED_URL_EXPIRY)
            for key in keys
        ]
    except Exception:
        logger.exception("Failed to generate signed URLs")
        return None
