...
def _build_image_urls(image_keys: Iterable[str]) -> list[str] | None:
    ...
    if not keys or not config.IMAGES_BUCKET_NAME:
        return None
    s3 = s3_client()
    try:
        return [
            s3.generate_presigned_url(
                "get_object",
                Params={"Bucket": config.IMAGES_BUCKET_NAME, "Key": key},
...
class AwsBackend:
    ...
    def create_post(self, body: CreatePostBody, user: UserInfo) -> dict:
        if not config.IMAGES_BUCKET_NAME:
            raise HTTPException(
    ...
    def delete_post(self, post_id: str, user: UserInfo) -> dict:
        ...
        if image_keys:
            ...
            for key in image_keys:
                try:
                    s3.delete_object(Bucket=config.IMAGES_BUCKET_NAME, Key=key)
                except Exception:
