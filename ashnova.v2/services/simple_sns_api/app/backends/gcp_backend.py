...
class GcpBackend:
    def list_posts(self, limit: int, next_token: str | None, tag: str | None) -> tuple[list[Post], str | None]:
        ...
        try:
            ...
            print(f"DEBUG: Found {len(docs)} posts in Firestore")
        except Exception as e:
            print(f"DEBUG: Firestore query failed: {e}")
            raise e
        ...
        for doc in docs:
            data = doc.to_dict() or {}
            print(
                f"DEBUG: Post data: {data.get('postId')} {data.get('createdAt')}")
            raw_image_keys = data.get("imageKeys")
            print(f"DEBUG: Raw imageKeys: {raw_image_keys}")

            image_keys = raw_image_keys or ([] if data.get(
    ...
    def create_post(self, body: CreatePostBody, user: UserInfo) -> dict:
        print(f"DEBUG: Creating post for user {user.user_id}")
        print(f"DEBUG: Incoming imageKeys: {body.imageKeys}")
        db = _get_firestore()
        post_id = str(uuid.uuid4())
        created_at = _now_iso()

        profile_doc = db.collection(
            settings.gcp_profiles_collection).document(user.user_id).get()
        ...
        print(
            f"DEBUG: Writing post {post_id} to Firestore collection {settings.gcp_posts_collection}")
        db.collection(settings.gcp_posts_collection).document(
            post_id).set(item)
        return {"item": item}

    def delete_post(self, post_id: str, user: UserInfo) -> dict:
        db = _get_firestore()
        post_ref = db.collection(
            settings.gcp_posts_collection).document(post_id)
        doc = post_ref.get()
        if not doc.exists:
    ...
    def get_profile(self, user: UserInfo) -> ProfileResponse:
        db = _get_firestore()
        doc = db.collection(settings.gcp_profiles_collection).document(
            user.user_id).get()
        if not doc.exists:
    ...
    def update_profile(self, body: ProfileUpdateRequest, user: UserInfo) -> ProfileResponse:
        ...
        profile_ref = db.collection(
            settings.gcp_profiles_collection).document(user.user_id)
    ...
    def create_upload_urls(self, body: UploadUrlsRequest, user: UserInfo) -> dict:
        ...
        for index in range(body.count):
            ...
            # When running on Cloud Run with default credentials, we must use IAM credentials API to sign
            # because the local credentials dont have a private key.
            # We use impersonated_credentials to "impersonate" the service account itself (or another one)
            # which provides a signing capable credential object.
            generate_url_kwargs = {}
            if settings.gcp_service_account:
                import google.auth
                from google.auth import impersonated_credentials

                # Get default credentials (token source)
                source_credentials, _ = google.auth.default()

                # Create impersonated credentials (signer)
