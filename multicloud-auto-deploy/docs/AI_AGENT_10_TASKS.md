...
All CI/CD pipelines:        ✅ Green (2026-02-21 commit 0ed0805)
...
| #   | タスク                            | 概要                                                                                                                                   |
| --- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 7   | **監視・アラート設定**            | CloudWatch Alarms (AWS) / Azure Monitor (Azure) / Cloud Monitoring (GCP) を設定。                                                      |
| 8   | **セキュリティ強化**              | CORS `allowedOrigins` を実ドメインに変更。GCP SSL 証明書の `example.com` プレースホルダを更新。Azure Key Vault ネットワーク ACL 強化。 |
| 9   | **WAF ログ集約**                  | 3 クラウドの WAF ログを一箇所に集約して統一視点を持つ。                                                                                |
| 10  | **Lambda Layer CI/CD 完全自動化** | Layer ビルド・公開時の non-fatal warning を解消。                                                                                      |
| 11  | **README 最終更新**               | 最新エンドポイント・認証挙動・CI/CD 状態を README に反映。                                                                             |
| 12  | **ブランチ保護ルール**            | `main` への直接 push を禁止。PR + CI パス必須にする。                                                                                  |
...
| #   | タスク                     | 概要                                                                                                                                 |
| --- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 13  | **カスタムドメイン設定**   | `aws.yourdomain.com` / `azure.yourdomain.com` / `gcp.yourdomain.com` を設定。[CUSTOM_DOMAIN_SETUP.md](CUSTOM_DOMAIN_SETUP.md) 参照。 |
| 14  | **負荷テスト**             | Locust などで性能ベースラインを確立。                                                                                                |
| 15  | **CI/CD 失敗通知**         | Slack / Discord webhook を追加。                                                                                                     |
| 16  | **テストカバレッジ拡充**   | 現在は最小限。E2E + 認証テストを追加。                                                                                               |
| 17  | **カオスエンジニアリング** | ネットワーク断・DB 停止・コールドスタートスパイクをシミュレート。                                                                    |
