...
def create_function_app_alerts(
    ...
    # Alert 1: High error rate (HTTP 5xx)
    alerts["http_5xx"] = azure_native.insights.MetricAlert(
        "function-5xx-alert",
        alert_rule_name=f"{project_name}-{stack}-function-5xx",
        resource_group_name=resource_group_name,
        description="Function App 5xx error rate is too high",
    ...
    alerts["response_time"] = azure_native.insights.MetricAlert(
        "function-response-time-alert",
        alert_rule_name=f"{project_name}-{stack}-function-latency",
        resource_group_name=resource_group_name,
        description="Function App response time is too high",
        severity=3,
        enabled=True,
        scopes=[function_app_id],
    ...
    alerts["memory"] = azure_native.insights.MetricAlert(
        ...
        alert_rule_name=f"{project_name}-{stack}-function-memory",
        resource_group_name=resource_group_name,
...
def create_cosmos_db_alerts(
    ...
    alerts["throttled_requests"] = azure_native.insights.MetricAlert(
        ...
        alert_rule_name=f"{project_name}-{stack}-cosmos-throttled",
        resource_group_name=resource_group_name,
    ...
    alerts["latency"] = azure_native.insights.MetricAlert(
        ...
        alert_rule_name=f"{project_name}-{stack}-cosmos-latency",
        resource_group_name=resource_group_name,
...
def create_frontdoor_alerts(
    ...
    alerts["error_percentage"] = azure_native.insights.MetricAlert(
        ...
        alert_rule_name=f"{project_name}-{stack}-frontdoor-errors",
        resource_group_name=resource_group_name,
